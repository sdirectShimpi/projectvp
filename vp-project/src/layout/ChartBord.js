...
const ChartBord = ({ sockets,userName, room }) => {
  const baseUrl = process.env.REACT_APP_API_URL;
  const [message, setMessage] = useState("");
  const [msgList, setMsgList] = useState([]);
  console.log("message",message)
  console.log("socket",sockets,'userName',userName,"room", room )
//   useEffect(() => {
...
const sendMessage = () => {
    if (message.trim() !== "") {
      ...
      sockets.emit("send-message", sentMessage);
      setMessage(""); 
    }
  };
  ...
  useEffect(() => {
    sockets.on("receive-msg", (data) => {
      setMsgList((list) => [...list, data.message]);
      sockets.off("receive-msg")
    });
  }, [msgList]);
